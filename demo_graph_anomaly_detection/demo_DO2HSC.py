...
from model.evaluation import test_stage_DO2HSC_loader
...
if __name__ == '__main__':
    ...
    for tc in np.unique(dataset.data.y):
        ...
        if args.eval == True:
            ...
            state_dict = torch.load('./weights/DO2HSC/'+DS+'_'+str(train_class)+'_model' + '.pth', map_location=device)
            
            model.load_state_dict(state_dict['model'])
            c = torch.Tensor(state_dict['c']).to(device)
            R_min = state_dict['R_min']
            R_max = state_dict['R_max']
            test_auc = test_stage_DO2HSC_loader(test_loader, model, c, R_max, R_min, device,
                                                                      train_class)
            ...
            for nu in [0.001,0.003,0.005,0.008,0.01,0.03,0.05,0.08]: # 
                ...
                with open('./result/DO2HSC_' + args.DS + '_result.txt', 'a') as f:
                    f.write(args.DS + '_train_class:' + str(args.train_class) + '\n')
