...
if __name__ == '__main__':
    ...
    if args.eval == True:
        ...
        state_dict = torch.load('./weights/DOHSC/'+datasetname+'_DOHSC' + '.pth')
        model.load_state_dict(state_dict['model'])
        c = torch.Tensor(state_dict['c']).to(device)
        R = state_dict['R']
        test_auc, test_f1 = load_model_DOHSC(test_loader, model, c, R, device)
        ...
        for nu in [0.001,0.003,0.005,0.008,0.01,0.03,0.05,0.08]:
            ...
            for rep in range(repNum):
                if not args.pretrain:
                    ...
                    c = torch.Tensor(state_dict['center']).to(device)
                    model=model.to(device)
                test_auc, _, _, f1_score = train_DOHSC(model, train_loader, test_loader, c, nu, epochs, args.normal_class, lr_milestones, lr, weight_decay, datasetname, device)
